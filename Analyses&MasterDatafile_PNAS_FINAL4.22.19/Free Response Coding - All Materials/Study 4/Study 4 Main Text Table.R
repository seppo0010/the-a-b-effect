...
##Codes by condition 
df.Inequality.summary <- summarySE(BDNT, measurevar = "Inequality", groupvars = "condLabel")

df.Consent.summary <- summarySE(BDNT, measurevar = "ConsentFinal", groupvars = "condLabel")

df.Experimentation.summary <- summarySE(BDNT, measurevar = "ExperimentationFinal", groupvars = "condLabel")

df.Randomization.summary <- summarySE(BDNT, measurevar = "RandomizationFinal", groupvars = "condLabel")
...
df.Codes.summaryPercent$condLabel <- mapvalues (df.Codes.summaryPercent$condLabel, from = c("Drug.A", "Drug.B", "Drug.AB"), to = c("Drug A", "Drug B", "AB Test"))
...
##Create CSV
# write_excel_csv (df.CodesVertical, "BDNTTable.csv") #write to table if commented in